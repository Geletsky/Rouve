...
if (laptopScreen.matches) {
	...
	tl
		.set('.header, .hero__container', { visibility: 'visible' })
		.set(".hero__button", { opacity: 0, scale: 0, })
		.from('.header__logo', .4, { x: -20, opacity: 0 })
		.from('.menu__list li', .4, { x: -20, opacity: 0, stagger: .2 }, '-=.2')
		.from('.menu__dark-mode', .4, { x: -20, opacity: 0 }, '-=.6')
		.from('.hero__mask-text', 1, { y: 100, opacity: 0, stagger: .2 }, '-=1.3')
		.from('.hero__decor', 1, { height: 0, opacity: 0 }, '-=1.3')
		.from('.hero__subtitle', 1, { y: 50, opacity: 0 }, '-=.7')
		.from('.hero__scroll', 1, { y: 20, opacity: 0 }, '-=.6')
		.to(".hero__button", .8, { y: -100, opacity: 1, scale: 1, ease: "power2.out" }, '-=1.3')
		.to(".hero__button", .7, { y: 0, ease: "bounce.out" }, '-=.8')
	...
	//! Section - portfolio
	const portfolioItemsLeft = document.querySelectorAll('.portfolio__items-left .item-portfolio');
	...
	const portfolioItemsRight = document.querySelectorAll('.portfolio__items-right .item-portfolio');
	...
	gsap.fromTo('.portfolio__button-pc', { x: 800, opacity: 0 }, {
		x: 0,
		opacity: 1,
		duration: 1.5,
		scrollTrigger: {
			trigger: '.portfolio__button-pc',
			start: '-110% center',
			toggleActions: "play restart play reverse"
		}
	});
	...
	//! Section - services
	gsap.fromTo('.services__button', { x: -50, opacity: 0 }, {
		x: 0,
		opacity: 1,
		duration: 1,
		scrollTrigger: {
	...
	gsap.fromTo('.item-services', { y: 50, opacity: 0 }, {
		y: 0,
		opacity: 1,
		duration: 1.5,
		stagger: .5,
	...
	//! Section - feedbacks
	gsap.fromTo('.item-feedbacks', { y: 50, opacity: 0, scale: 0 }, {
		y: 0,
		opacity: 1,
		scale: 1,
		duration: 1.5,
	...
	gsap.fromTo('.buttons-feedbacks', { x: 100, opacity: 0 }, {
		x: 0,
		opacity: 1,
		duration: 1,
		scrollTrigger: {
...
if (touchScreen.matches) {
	...
	tl
		...
		.from('.hero__decor', 1, { height: 0, opacity: 0 }, '-=1.3')
		.from('.hero__subtitle', 1, { y: 50, opacity: 0 }, '-=.7')
		.to(".hero__button", .8, { y: -50, opacity: 1, scale: 1, ease: "power2.out" }, '-=1.3')
		.to(".hero__button", .7, { y: 0, ease: "bounce.out" }, '-=.8')
	...
	gsap.fromTo('.portfolio__button-touch', { x: 100, opacity: 0 }, {
		x: 0,
		opacity: 1,
		duration: 1.5,
		scrollTrigger: {
			trigger: '.portfolio__button-touch',
			start: '-200% center',
			toggleActions: "play restart play reverse"
		}
	});
